...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt
        .setName('bot_id')
        .setDescription('The Bot ID to start (use /mybots to see your IDs)')
        .setRequired(true)
    ),
  ...
  async execute(interaction) {
    ...
    if (bot.expiresAt && Date.now() > bot.expiresAt) {
      return interaction.editReply({
        content: '❌ Your bot subscription has expired. Please contact the admin to renew.',
      });
    }
    ...
    try {
      ...
      interaction.editReply({ content: `❌ Failed to start: \`${err.message}\`` });
    }
  },
};

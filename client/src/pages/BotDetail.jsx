...
export default function BotDetail() {
  ...
  const fetchBot = async () => {
    try {
      ...
      setEditExpiry(
        data.expiresAt
          ? new Date(data.expiresAt).toISOString().slice(0, 16) // datetime-local format
          : ''
      );
    } catch {
      navigate('/dashboard');
  ...
  return (
    <>
          ...
          <div className="card space-y-5">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button
                ...
                disabled={!!busy}
                onClick={() => setConfirm({ action: 'update' })}
              >
                {busy === 'update' ? '…' : '⬆️ Pull & Update'}
              </button>
              <button
      ...
      {confirm?.action === 'update' && (
        <ConfirmModal
          title="Pull & Update Bot"
          message="This will run git pull + npm install, then restart the bot. Any local changes in the bot directory will be overwritten."
      ...
      {confirm?.action === 'delete' && (
        <ConfirmModal
          ...
          message="This will stop the bot, remove it from PM2, and permanently delete its source directory. This cannot be undone."

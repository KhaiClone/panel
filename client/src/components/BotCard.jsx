...
const STATUS_STYLES = {
  ...
  errored:   { dot: 'bg-orange-400',  text: 'text-orange-400',  label: 'Errored'  },
  launching: { dot: 'bg-yellow-400',  text: 'text-yellow-400',  label: 'Starting' },
};

const getStyle = (s) => STATUS_STYLES[s] ?? { dot: 'bg-slate-500', text: 'text-slate-400', label: s ?? 'Unknown' };
      ...
      <div className="card flex flex-col gap-4 hover:border-slate-600 transition-colors">
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <p className="text-xs text-slate-500 font-mono truncate">{bot.buyerID} / {bot.botID}</p>
          </div>
          {/* Status badge */}
          <div className={`flex items-center gap-1.5 shrink-0 text-xs font-medium ${style.text}`}>
            <span className={`w-2 h-2 rounded-full ${style.dot} ${isOnline ? 'animate-pulse' : ''}`} />
            {style.label}
          </div>
        </div>
        ...
        {/* Expiry */}
        {msLeft !== null && (
          <div className={`text-xs px-2.5 py-1.5 rounded-lg w-fit ${
            msLeft <= 0
        ...
        {bot.live?.status === 'online' && (
          <div className="flex gap-4 text-xs text-slate-500">
            <span>CPU: <span className="text-slate-300">{bot.live.cpu ?? 0}%</span></span>
            <span>RAM: <span className="text-slate-300">
              {bot.live.memory ? `${(bot.live.memory / 1_048_576).toFixed(0)} MB` : '—'}
            </span></span>
            <span>↺ <span className="text-slate-300">{bot.live.restarts ?? 0}</span></span>
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-2 pt-1 border-t border-slate-700">
          {isStopped && (
            <button className="btn-success text-xs py-1.5 px-3" onClick={() => action('start')} disabled={busy}>
              ▶ Start
            </button>
          )}
          {isOnline && (
            <button className="btn-danger text-xs py-1.5 px-3" onClick={() => action('stop')} disabled={busy}>
              ⏹ Stop
            </button>
          )}
          <button className="btn-warning text-xs py-1.5 px-3" onClick={() => action('restart')} disabled={busy}>
            🔄 Restart
          </button>
          <button className="btn-ghost text-xs py-1.5 px-3" onClick={() => navigate(`/bots/${bot._id}`)} disabled={busy}>
            🔍 Details
          </button>
          <button
